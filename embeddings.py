...
def ensure_collection(client: QdrantClient, collection_name: str, vector_size: int):
    """Create or recreate collection if needed. Uses cosine distance."""
    print(f"Ensuring collection '{collection_name}' exists with vector size {vector_size}")
    # If exists and vector size differs, recreate; otherwise create if missing.
    try:
        info = client.get_collection(collection_name=collection_name)
        current_size = info.vectors_config.size
        if current_size != vector_size:
            print(f"Collection exists but vector size mismatch ({current_size} != {vector_size}). Recreating.")
            client.recreate_collection(
                collection_name=collection_name,
                vectors_config=VectorParams(size=vector_size, distance=Distance.COSINE),
            )
    except Exception:
        # Not found or other; create
        client.recreate_collection(
            collection_name=collection_name,
            vectors_config=VectorParams(size=vector_size, distance=Distance.COSINE),
        )
